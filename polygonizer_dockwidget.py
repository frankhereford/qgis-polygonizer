        ...
        for road in selected_features_layer.getFeatures():
            ...
            if interconnect_length > 0.0000001: # this is about 2500 hydrogen atoms long; this is very zero in the scope of a road
                ...
                for i in range(subsections[0]):
                    ...
                    # these are to cases to handle when the road ends not at an intersection, one for each potential non-intersectional endpoint
                    # we're going to buffer that point and join it to the last polygon so we get a fake round end cap but only on that one end
